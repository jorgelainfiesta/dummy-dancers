/* 
 * From: http://www.airtightinteractive.com/2013/10/making-audio-reactive-visuals/
 */
define(['./data'], function(data){
	...
	var sampleAudioURL = "audio/atoms.mp3";
...
//		events.on("update", update);
	...
	function loadSampleAudio() {
		...
		request.open("GET", data.get('audioParams.sampleURL'), true);
	...
	function onUseMic(){
		...
		if (data.get('audioParams.useMic')){
	...
	function onUseSample(){
		if (data.get('audioParams.useSample')){
			loadSampleAudio();          
			data.set('audioParams.useMic', false);
		}else{
			stopSound();
		}
	}
...
//		events.emit("onBeat");
	...
	function update(){
		...
		for(var i = 0; i < binCount; i++) {
			waveData[i] = ((timeByteData[i] - 128) /128 )* data.get('audioParams.volSens');
		}
		...
		for(var i = 0; i < levelsCount; i++) {
			...
			levelsData[i] = sum / levelBins/256 * data.get('audioParams.volSens'); //freqData maxs at 256
		...
		if (level  > beatCutOff && level > BEAT_MIN){
			...
			if (beatTime <= data.get('audioParams.beatHoldTime')){
				...
				beatCutOff *= data.get('audioParams.beatDecayRate');
